...
### TASK-015: 핵심 기능 통합 테스트 `TODO`

> 예상 소요: 1.5일 · 기능: 전체 플로우 통합 테스트

Playwright를 활용한 E2E 통합 테스트로 주요 사용자 플로우 검증

1. **주최자 플로우**: 이벤트 생성 → 초대링크 공유 → 참여자 확인
   - 이벤트 생성 (제목, 설명, 장소, 후보 날짜, 마감일)
   - 초대 링크 복사
   - 대시보드에서 이벤트 확인
   - 참여자 목록 및 히트맵 확인
...
**테스트 파일**: `tests/integration-flow.spec.ts`
